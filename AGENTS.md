...
- Start and replay in TypeSafe API mode (user correction 2026-09-20). Only explicit Play starts the three decisions; loading the page or choosing a mode never calls TypeSafe. Keep demo as a manual option, clearly label its fictional data, and never silently fall back to demo after an API failure. Key presence is not proof of successful authentication or credit balance.
...
- Keep the production interface minimal: question / premise, cockpit, two groups, Play, Jev choice with probabilities/confidence, terminal result and replay. Do not restore fictional polls, slogans, redundant captions or developer setup instructions in the game screen. Both decision modes share the same screen; show the compact fabricated-demo badge and keep optional mode controls in its disclosure (2026-09-20).
